...
// Migrate models from the registry
func migrateRegisteredModels(db *gorm.DB) {
	for _, model := range database.MigratableModels {
		modelName := fmt.Sprintf("%T", model)

		// Check if already applied (optional, based on your tracking system)
		var count int64
		if err := db.Model(&models.Migration{}).Where("name = ?", modelName).Count(&count).Error; err != nil {
			log.Fatalf("Error checking applied migrations: %v", err)
		}
		...
		fmt.Printf("Running migration for model: %s\n", modelName)
		...
		// Record as applied
		if err := db.Create(&models.Migration{Name: modelName}).Error; err != nil {
			log.Fatalf("Failed to record migration for %s: %v", modelName, err)
		}
	}
}
